...
# Iter arch-2a — `_RULES` + `_scan_text` relocated verbatim to
# services/security_text_scanner.py. `services/loop_engine.py` needed
# these for its ship-time inline scan and previously imported them
# FROM this router — an inverted (service→router) dependency flagged
# by services/architecture_health.py's boundary scan. Re-imported here
# so every existing call site in this file keeps working unchanged.
from services.security_text_scanner import _RULES, _scan_text        # noqa: E402
...
# `_scan_text` (+ its `_RULES` table) is now imported from
# services/security_text_scanner.py above — see that module for the
