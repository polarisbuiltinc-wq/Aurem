...
# Max request body size (bytes) for /auth/signup — prevents large-payload
# DoS. Env-tunable so prod can adjust without a code change.
_SIGNUP_MAX_BODY = int(os.getenv("SIGNUP_MAX_BODY_BYTES", "16384"))
...
@router.post("/signup")
async def signup(request: Request, body: SignupBody) -> dict:
    # Reject oversized payloads early to prevent large-body DoS.
    content_length = request.headers.get("content-length")
    if content_length and int(content_length) > _SIGNUP_MAX_BODY:
