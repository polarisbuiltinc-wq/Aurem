...
# Max bytes accepted on /auth/signup so an attacker can't OOM the
# JSON parser with a multi-MiB password field. Env-tunable.
_MAX_SIGNUP_BODY_BYTES = int(os.getenv("AUTH_MAX_SIGNUP_BODY_BYTES", "1048576"))
...
async def signup(body: SignupBody, request: Request) -> dict:
    # Reject oversized payloads before any DB work so an attacker
    # can't use a multi-MiB body to exhaust the JSON parser or Mongo.
    cl = request.headers.get("content-length")
    if cl is not None and int(cl) > _MAX_SIGNUP_BODY_BYTES:
