...
async def create_project(
    ...
    Iter 181 — TestSprite caught a recurring CF 520/502 on this route
    in production ("origin web server returned an invalid or incomplete
    response"). Root cause: the combined wall-clock of LLM (up to 80s)
    + GitHub push (~10-15s) + Mongo provision (~5-10s) could exceed
    Cloudflare's 100s edge limit. Hardened by:
      1. tightening the LLM cap 80s → 45s,
      2. wrapping GitHub push + DB provision in asyncio.wait_for so
         no single step can ever stall the request,
      3. a final `except Exception` net so unexpected failures return
         clean 502 JSON instead of a worker-crash CF page.
    """
    ...
    try:
        ...
        raise HTTPException(504, {
            ...
            "msg":  "Code generation took longer than 45s. The LLM is likely "
    ...
    try:
        ...
        logger.warning("[projects] GitHub push exceeded 20s — returning non-fatal")
        result["github"] = {"ok": False, "error": "GitHub push timed out after 20s"}
    ...
    if body.provision_db:
        try:
            db = await asyncio.wait_for(
                provision_project_db(project_id),
                timeout=20.0,
            )
            result["database"] = {
                "ok": db["ok"],
            ...
            logger.warning("[projects] DB provision exceeded 20s — non-fatal")
            result["database"] = {"ok": False, "error": "DB provision timed out after 20s"}
        except Exception as e:
