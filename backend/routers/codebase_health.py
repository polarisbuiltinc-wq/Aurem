...
def _security_fix_hint(rule_id: str) -> str:
    mapping = {
        ...
        "eval_usage":        "Replace the dynamic evaluation call with `ast.literal_eval` or an explicit parser.",
        "exec_usage":        "Replace the dynamic execution call with a safer dispatch (dict of callables).",
    ...
    for path, text in text_cache.items():
        ...
        if path.endswith(".py"):
            ...
            for i, (start, name) in enumerate(fn_starts):
                end = fn_starts[i+1][0] if i+1 < len(fn_starts) else