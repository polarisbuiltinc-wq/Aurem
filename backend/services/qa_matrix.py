...
# ═══════════════════════════════════════════════════════════════════
# Iter arch-2a — relocated VERBATIM from routers/admin_qa.py (no
# logic changes). services/health_checks.py needed `_harvest_counts`
# + `_harvest_ci_status` for its CI-drift health check and previously
# imported them FROM the router — an inverted (service→router)
# dependency flagged by services/architecture_health.py's boundary
# scan. routers/admin_qa.py re-imports these below for its own
# /admin/qa/counts and /admin/qa/status endpoints.
# ═══════════════════════════════════════════════════════════════════
from pathlib import Path as _Path
import httpx
from services.http import ext_client

_QA_APP_ROOT = _Path("/app")

_TEST_FN_RE_PY  = re.compile(r"^\s*(?:async\s+)?def\s+test_\w+\s*\(", re.M)
_TEST_FN_RE_JS  = re.compile(r"^\s*(?:test|it)\s*\(\s*[\"']", re.M)


def _count_matches(paths: list, regex) -> int:
    total = 0
    for p in paths:
...
def _glob_files(root, patterns: list) -> list:
    out: list = []
    for pat in patterns:
...
def _harvest_counts() -> dict:
    ...
    backend_pytest_files = len(list(
        (_QA_APP_ROOT / "backend" / "tests").glob("test_*.py")
    )) + len(list(
        (_QA_APP_ROOT / "backend" / "tests" / "reasoning").glob("test_*.py")
    ))

    # ── Frontend Vitest (component RTL + a11y) ───────────────────
    frontend_vitest_files = list(
        (_QA_APP_ROOT / "frontend" / "src").rglob("*.test.jsx")
    ) + list(
        (_QA_APP_ROOT / "frontend" / "src").rglob("*.test.js")
    )
    frontend_vitest_total = _count_matches(frontend_vitest_files, _TEST_FN_RE_JS)

    # ── Playwright ───────────────────────────────────────────────
    playwright = list(
        (_QA_APP_ROOT / "frontend" / "tests" / "visual").glob("*.spec.js")
    )
    ...
    if backend_pytest_files == 0:
        try:
            mpath = _QA_APP_ROOT / "backend" / "qa_manifest.json"
