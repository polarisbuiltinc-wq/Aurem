Looking at the finding, the regex `\bdangerouslySetInnerHTML\b` is too broad—it matches its own rule definition in this `.py` file, causing a self-match false positive. The fix is to make the regex more specific to actual JSX usage patterns where `dangerouslySetInnerHTML` is used as a prop assignment.
...
_VULN_RULES: list[tuple[str, re.Pattern, str, str]] = [
    ...
    ("dangerously_set_html",
     re.compile(r"\bdangerouslySetInnerHTML\s*=\s*\{"),
     "HIGH",
