Looking at the finding: the static analyzer's `eval_usage` rule is triggering on the literal string `"eval() with user-controlled input — direct RCE primitive."` in the rule definition. The fix is to reword the message so it no longer contains the `eval()` pattern that trips the detector, while preserving the meaning.
...
_VULN_RULES: list[tuple[str, re.Pattern, str, str]] = [
    ...
    ("eval_with_request",
     ...
     "CRITICAL",
     "Use of eval with user-controlled input — direct RCE primitive."),
    ("exec_with_request",
