<think>
The finding indicates a missing compound rule `chain_eval_plus_secret` that should detect when dynamic code execution sinks (eval/exec) co-occur with hardcoded secrets in the same file. I need to add this compound detection to the `scan_bug_hunt` function, right before the `return out` statement.
"""
