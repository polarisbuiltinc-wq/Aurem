...
Review the patch for ALL of these dimensions:
  ...
  2. Code injection — do NOT use eval, exec, shell=True, dynamic imports, sql
...
SEVERITY RULES (Iter 212m-41 — production calibration):
  CRITICAL = an attacker can OWN the system or read other users' data
             ...
             real hard-coded API key, direct use of user input in
             dangerous functions, SQL string concatenation on a
             user-controlled fragment, an unauth'd admin route.
             Be SURE before you mark anything critical.
