...
Review the patch for ALL of these dimensions:
  ...
  2. Code injection — avoid eval, exec, shell=True, dynamic imports, sql
  ...
  5. Cross-site scripting — avoid innerHTML, dangerouslySetInnerHTML with
     untrusted data, raw html.unescape on user input, jQuery .html() with
     user data
...
SEVERITY RULES (Iter 212m-41 — production calibration):
  CRITICAL = an attacker can OWN the system or read other users' data
             ...
             real hard-coded API key, direct use of untrusted input in
             eval/exec, SQL string concatenation on a user-controlled
             fragment, an unauth'd admin route.  Be SURE before you
             mark anything critical.
