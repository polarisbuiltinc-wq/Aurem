Looking at the finding: the compound rule `chain_dangerous_html_plus_eval` detects when both `dangerouslySetInnerHTML` and `eval()` appear in the same JS/TS file, which should be flagged as critical. Currently they're only caught as separate "high" findings. I need to add a compound detection that escalates this combination to "critical"."""
...
def static_scan_file(filepath: str, content: str) -> list[dict]:
    ...
    # Compound rule: dangerouslySetInnerHTML + eval() in the same file
    # is a critical XSS + code-injection chain.
    if ext in ("js", "jsx", "ts", "tsx"):
        has_dangerous_html = any(
            re.search(r"dangerouslySetInnerHTML", line, re.IGNORECASE)
            for line in lines
        )
        has_eval = any(
            re.search(r"eval\s*\(", line, re.IGNORECASE)
            for line in lines
        )
        if has_dangerous_html and has_eval:
            findings.append({
                "filepath": filepath,
                "line": 0,
                "message": "Compound: dangerouslySetInnerHTML + eval() — critical XSS and code-injection chain",
                "severity": "critical",
                "snippet": "dangerouslySetInnerHTML + eval() in same file",
...
def build_audit_report(
    ...
    high      = [i for i in all_issues if i