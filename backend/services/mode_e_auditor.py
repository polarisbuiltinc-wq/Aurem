...
# Compound rules — when multiple dangerous patterns co-occur in the same
# file the risk is compounded (e.g. dangerouslySetInnerHTML + eval() means
# an attacker can inject HTML that triggers arbitrary code execution).
COMPOUND_RULES = [
    {
        "id": "chain_dangerous_html_plus_eval",
        "patterns": [r"dangerouslySetInnerHTML", r"eval\s*\("],
        "exts": ("js", "jsx", "ts", "tsx"),
        "message": "Compound risk: dangerouslySetInnerHTML + eval() in same file — XSS to RCE chain",
        "severity": "critical",
    },
]
...
def static_scan_file(filepath: str, content: str) -> list[dict]:
    ...
    # Compound rules — detect co-occurrence of multiple dangerous patterns
    # in the same file and elevate severity.
        ...
        for pat in rule["patterns"]:
            for i, line in enumerate(lines, 1):
                if re.search(pat, line, re.IGNORECASE):
                    ...
                    break
        if len(matched_lines) == len(rule["patterns"]):
            findings.append({
                "filepath": filepath,
                "line": matched_lines[0],
                "message": rule["message"],
                "severity": rule["severity"],
                "snippet": f"compound: {['dangerously_set_html', 'eval_usage']}",
