...
# Compound / chained patterns — when two dangerous primitives co-occur
# in the same file the risk is elevated to critical.
COMPOUND_PATTERNS = [
    (
        ["dangerouslySetInnerHTML", r"eval\s*\("],
        "compound: ['dangerously_set_html', 'eval_usage']",
        "critical",
    ),
]
...
def static_scan_file(filepath: str, content: str) -> list[dict]:
    ...
    # Compound checks — multiple dangerous primitives in the same file.
    if ext in ("js", "jsx", "ts", "tsx"):
        for sub_patterns, message, severity in COMPOUND_PATTERNS:
            matched_lines = []
            for sp in sub_patterns:
                for i, line in enumerate(lines, 1):
                    if re.search(sp, line, re.IGNORECASE):
                        ...
                        break
            if len(matched_lines) == len(sub_patterns):
                findings.append({
                    "filepath": filepath,
                    "line": matched_lines[0],
                    "message": message,
                    "severity": severity,
                    "snippet": lines[matched_lines[0] - 1].strip()[:100],
                    "source": "static",
                })
