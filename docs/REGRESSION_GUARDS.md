...
| 19 | Process auto-recovery | Crash loops hiding real bugs | ✅ SHIPPED (Iter 362) — supervisor autorestart=true (pod conf) + services/process_recovery.py: boot recording (process_boots), 3-boots-in-10min loop detection → CRITICAL row in EXISTING topup_alerts banner + auto-resolve when stable; /api/healthz heartbeat; GET /admin/qa/guard19-recovery; locks test_iter362 (10) |
| 20 | Automated postmortem log | Incidents without history/patterns | ✅ SHIPPED (Iter 363) — services/incident_log.py auto-creates incidents from ANY guard's RED/critical alert (G19 loop + integration_health critical hooks), auto-resolves + MTTR on clear; /admin/qa "Incident Log" section (filter open/resolved, MTTR 30d); GET /admin/qa/guard20-incidents; locks test_iter363 (8) |
