...
export function activate(context: vscode.ExtensionContext) {
  token = context.globalState.get<string>('aurem.token');
  ...
  context.subscriptions.push(
    ...
    vscode.commands.registerCommand('aurem.login', async () => {
      ...
      const gotToken = new Promise<string>((resolve, reject) => {
        ...
        async () => {
          try {
            ...
            const action = await vscode.window.showInformationMessage(
              `ORA is working! Task ${result.task_id || ''}`, 'Open AUREM');
...
function getSidebarHtml(serverUrl: string, tk: string | undefined): string {
  if (tk) {
