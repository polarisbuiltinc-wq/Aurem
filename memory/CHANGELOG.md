...
- **Guards Fix Batch — G1/G7/G12/G15/G21, preview-verified (2026-08-19)** — all 5 code-bug guards fixed and confirmed live via `/admin/status/all`:
  ...
  - **G21 Security Scan** → red (correctly — REAL finding: 1 unpinned dependency in requirements.txt). Adapter was querying `db.vanguard_findings` (scanner:"trufflehog") — a collection nothing writes (that's the CUSTOMER-repo CI ingest table, unrelated to G21). Now calls the same live `run_scan()` the working `/admin/qa` endpoint already uses.
 ...
 — founder caught "GLM 5.2" visible directly in the chat UI (not just admin panels). Full scan of every place a raw provider/model string reaches a regular (non-admin) user:
