...
- **Wave 1 (guardrail remediation) CLOSED + test-baseline honesty triage (2026-09-08)**
  ...
  - **Test-baseline honesty pass (option-3, scoped):** ran the full suite (402 failed/6603 passed/16 errors, was NOT the founder-approved "regression-green" definition — diffed against the 2026-08-28 baseline). Of the 128 test-IDs failing-but-not-in-that-baseline:
    - **2 REAL BUGS found + fixed** (not blind-fixed — root-caused first): (1) `routers/cto_projects.py::submit_task` — project-ownership check now runs BEFORE the ambiguity/clarify gate, so an unknown/foreign `project_id` returns 403 instead of a 200 `needs_clarification`. (2) `routers/auth.py::/auth/me` — was leaking `mfa_secret_pending` (plaintext TOTP secret) + `mfa_backup_codes_pending` (bcrypt hashes) for accounts with an unfinished 2FA enrollment; live-verified leak on `test_admin_001`, fixed, re-verified clean via curl.
    - **~14 TEST BUGS fixed** (stale paths after the Parliament-package split, the admin.py Phase-2 split, the 2026-08-13 brand rebrand, the 2026-08-31 quota-gate-replaces-paywall change, and 2 hardcoded-repo-URL test-isolation collisions).
    - **2 fixture-drift fixes**: `test_health_notifier.py` + `test_iter212m30_pr2_founder_indexing.py` fake Mongo collections were missing `.with_options()`, which commit `36afb6cc` added to 3 real production write sites (Atlas WTimeoutError fix). Added passthrough stubs — fixed 11 tests.
    - **1 dead test quarantined**: `test_ora_chat_deep_research.py` (Auto Deep-Research HTTP wiring was removed in the "ORA Chat v2 rebuild", commit `2ce66e67`).
    - **4 REAL BUG SUSPECTS ticketed in ROADMAP.md P0.5** (not blind-fixed, not silently quarantined): secret/key-detection cluster (3 files, security-relevant), ORA Chat v2 mock-LLM-stream not emitting `final` events, `_DB` fixture missing `loop_execution_log`, code-invariant-lock drift (2 files).
    - Remaining ~35 of the 128 were batch-classified as env-noise (live GitHub/Stripe/LLM calls unavailable in sandbox) or full-suite-run-only flakiness (confirmed passing in isolation) — not individually root-caused, per founder's explicit "don't chase 406, that's a time-sink" instruction.
  - `backend/test-baseline.txt` refreshed to today's true snapshot (404 entries: 388 FAILED + 16 ERROR; 101 old entries removed as now-passing, 95 new entries added, same founder-ruled "regression-green = no NEW failing IDs vs this file" gate definition retained). See file header for full detail.


- **R5e webhook — CLOSED (founder-verified in GitHub's own UI, 2026-08-29/30, same fork) · Receipt UI dual-shot polish SHIPPED · NEW workstream: ORA Admin web-inspect tools SHIPPED**
  - **R5e CLOSED.** Took 3 rounds to actually land (two false "saved" beliefs corrected along the way: the `pull_request` event checkbox wasn't actually committed the first time — Save was never clicked; the webhook secret re-save also silently failed once). Root cause of the agent's OWN verification gap: this preview pod's own `admin_settings.github_app_config` (separate Mongo from production) had a stale/invalid private key that started 401'ing against GitHub's `/app` API entirely, so the agent could not independently re-verify via its own API call — founder declined to also update preview's copy. Final evidence, gathered by the **founder directly in GitHub's UI** (agent talked them through it, never touched/received the secret):
    1. Redeliver of the most recent delivery → **HTTP 200** (secret now matches).
    2. New throwaway PR (#385, fork `polarisbuiltinc-wq/Aurem-1` → `TJSNDHU/Aurem`) → fresh `pull_request.opened` delivery → **HTTP 200**.
    3. Cleanup done: PR #385 closed, `patch-1` branch deleted — no orphan branch/PR left.
  - Secret value never transited chat, logs, or any file this agent touched (repeatedly refused when offered/pasted). R9 was NOT touched or flipped by this work.
  - **Receipt UI polish (SHIPPED, tested)**: `DeployPanel.jsx`'s `ReceiptCard` now shows the viewport AND full-page shots side by side, labelled "Viewport" / "Full page" (`data-testid="deploy-receipt-shot-viewport"` / `"-fullpage"`), back-compat single-shot fallback for old runs with no fullpage key. Backend: `GET /deploy/runs/{run_id}/receipt?variant=viewport|fullpage` (new `variant` param, 404 if the requested variant's key is missing — never silently substitutes), `GET /deploy/log/{run_id}` now also returns `fullpage_receipt_key`. Tests: `backend/tests/test_receipt_dual_shot_2026_08_30.py` (incl. `test_receipt_shows_both_shots`), `frontend/src/components/__tests__/DeployPanel.ReceiptCard.test.jsx` (incl. `t_receipt_shows_both_shots`) — all pass via testing_agent. **Known infra gap (not fixed, out of scope)**: this preview pod has no `R2_ENDPOINT` configured, so `upload_receipt` can't actually store real screenshot bytes here — verified via unit/component tests with mocked images, not a live end-to-end real-image render.
  - **NEW workstream — ORA Admin web-inspect tools (SHIPPED, tested, parallel to R9, does not touch it)**: `services/web_inspect.py` (new) — `web_verify` (deterministic, ZERO LLM, direct reuse of V1a `deploy_verify.run_verify` against ANY external http/https URL, no project lock) and `web_inspect` (pruned page-text snapshot capped at `SNAPSHOT_CHAR_CAP`, wrapped in a nonce-marked `PAGE_CONTENT` untrusted-content boundary, advisory-only plain-English answer ≤300 words via OpenRouter model `qwen/qwen3.8-27b` — confirmed live against `GET openrouter.ai/api/v1/models` on 2026-08-30, not guessed). SSRF-fenced by reusing the existing `deep_research._is_safe_public_url` gate (holds even for the admin — a 169.254.169.254 target is refused + audit-logged). Metered via the existing `services.llm_usd_cap` global-kill-switch module (per-plan $ cap already skipped for founder/admin tier by that module's pre-existing logic, nothing new built there) + `services.ora_chat.cost_tracker.log_call`. Fresh Playwright `BrowserContext` per call, no login/credential/cookie storage (explicitly out of scope this round, per spec). Wired into the existing ORA Admin chat function-calling catalog (`services/ora_chat_v2/tools.py`, now 8 read-only tools; `engine.py` now threads `user_id` into `execute_tool`). Tests: `backend/tests/test_web_inspect_2026_08_30.py` (8 named tests: zero-LLM, V1a reuse, OpenRouter-call, snapshot-pruned-bounded, SSRF-fenced, metered, no-credentials, advisory-only) — all pass via testing_agent. No REST endpoint added — tools are chat-only (admin phrases like "check if &lt;url&gt; deployed OK" / "what's on this page?"). No new dependencies. R9 untouched.
  - Regression: 100 backend tests pass (deploy_verify/v1d/receipt/ora_chat/capability-manifest suite) + 11 new (receipt+web-inspect) + 7 new frontend vitest, all via testing_agent; 3 known pre-existing `ora_chat_v2` failures reproduced unchanged (not regressions, confirmed via `git stash`).
  - **STOPPING per founder instruction, awaiting the exact "GO R9"** — R9 has not been flipped.


- **Overnight Master Loop — W1/W2/W3 done/blocked/reported (2026-08-29, new fork)** — full detail `/app/memory/REPORT-final-loop.md`. **W2 P0-B closed for UI/code**: zero-LLM instrumented trace proved the backend never returns truly-empty content for the "approve after a pending fix" repro; real root cause was `MessageBubble.jsx`'s `FILE_PATH_TOKEN` (Gate 6) mandating a `/` in the path, so any fence confined to a repo-ROOT file (README.md etc — this codebase's own go-to test example) could never render the Approve button. Fixed (slash now optional), 4 new tests + 13 pre-existing re-run 17/17, 0 regressions. **W2 Step 2**: shipped the MOCK_LLM short-circuit at the top of `chat_stream` (the live main-chat path had never honored it before); live-curl zero-spend proof (`ora_chat_usage` 1467→1467 across 5 real calls) + screenshot. **W3 S1-S5**: S1-P4 URL auto-detect (`GET /cto/projects/{id}/detect-live-url`, checks vercel.json/netlify.toml/package.json homepage), S2 "What changed" default Code view (`GET /cto/projects/{id}/what-changed`, deterministic server/UI classification + top-5 diff) with the existing file browser moved behind an "All files" sub-tab, S3 Deploy "Go live" rename + last-look confirmation modal + honest receipts card + rollback-disabled-when-none state (config-form field live-verified; modal not live-clicked — this pod has no `AUREM_CTO_MASTER_KEY` vault secret), S4 new "Preview & Deploy Monitor" admin tile next to Webhook Fence, S5 30-day meter line + zero-LLM guard tests + `ROADMAP.md` F25 entry. testing_agent integrated pass: 100%/100%, 0 bugs. New backend module `services/trust_surface_events.py` (lightweight event log, no existing generic sink found to extend). No prod flip, no baseline edits, no new deps.

- **Phase 3 continuation — Kit Phase B v1 slice + Track B evidence-wall report (2026-08-28, same fork)** — full detail `/app/memory/LOOP-STATE.md`'s "PHASE 3 continuation" section. Built `routers/visibility.py` + `services/visibility/*` (detect/robots/schema/sitemap/apply) + `migrations/003_visibility_kit.py`, reusing the same P6 branch+PR primitives T7 already proved live; 10 new tests pass, live-curl-verified. Scope: only the 3 zero-LLM AUTO items (robots/schema/sitemap) are wired; llms.txt + preferred_sources generators + frontend panel are next. Added `kit_day14_reminder` health check (Q2) reusing the existing notifier cron, 4 tests pass. Documented precisely (Q3): the broken GitHub webhook blocks nothing built in Kit today. Track B (reliability diagnosis): found Preview's DB contains zero matches for the founder's 5 named production failures — all Preview failure data confirmed to be test/QA fixture noise (auto-generated demo projects, old drill harnesses, stale pre-migration errors, or expected user-cancel/timeout, not bugs). Reported the evidence wall honestly instead of guessing a root cause; asked founder for the real production error payloads.
...
## 2026-08-28 (continuation) — P0 production hotfix: commit_files() missing-author crash + rollback approve-button gap

Founder-reported live production regression (root-caused end-to-end, not just patched):

1. **`commit_files()` missing-args TypeError** — `routers/cto_projects.py::_run_task_via_api` (the no-git-binary ship engine) called `gh_api_commit()` without ever resolving `author_name`/`author_email` — the ONLY commit_files call site in the repo missing this (every other caller already resolves identity first). Fixed: resolves via `resolve_git_identity(db, user_id)` before the call, matching the existing convention everywhere else.
2. **Misleading "update your profile" error copy** — traced to `services/cto_projects_helpers.py::_set_status()` sending the RAW exception text to an LLM rewriter (`error_translator.py`) on any task failure, no guard for internal bugs. Since the raw TypeError text literally contains "author_email"/"author_name", the LLM plausibly invented profile-update guidance. Fixed: when `error_code == "INTERNAL_CALL_ERROR"`, uses the correct catalog message from `core/errors.py::translate_error()` directly, bypassing the LLM entirely.
3. **Rollback/revert never got an Approve button** — the persona (`services/orchestrator.py::AUREM_CTO_PERSONA`) had zero mutation-verb recognition for "revert"/"rollback"/"undo", so the LLM correctly (per its own anti-hallucination rules) never emitted the `aurem-handoff` fence for these requests — no fence, no button, and (by design, Iter 212m-26) a bare "yes" correctly does NOT auto-ship. Fixed: added revert/rollback/undo/restore to the mutation-verb whitelist + a dedicated guidance section teaching the LLM to use `get_commit_diff`/`read_repo_file` to identify the exact prior content and emit the fence instead of asking permission in prose.
4. **Frontend/backend verb-whitelist drift (found by testing_agent)** — backend fence now says "revert" but `frontend/src/components/MessageBubble.jsx`'s independent `MUTATION_VERBS` regex didn't include it, so `extractHandoffBrief` rejected the fence and the fallback "approve button didn't load" alert fired instead. Fixed: added revert/rollback/undo/restore there too, exported the constant, added a locking Vitest test.
5. Minor: council "double-checked by a second reviewer" trust badge could render on an empty assistant bubble; now gated on `m.content` being non-empty.

**Testing**: 6 new backend pytest (all pass) + 2 new frontend Vitest (all pass). Regression at the time: 288 pre-existing backend tests (0 new failures) + 558 frontend tests (0 new failures) — **that 288 was a targeted subset, not the full suite.** `testing_agent` iteration `iteration_p0_2026_08_28_commit_files_and_rollback.json` found gap #4 above, which was then fixed and locked with a test.

**CORRECTION (2026-08-28, audit pass, memory/RECON-LEDGER.md)**: the "0 new failures" framing above was true only for the narrow subset actually run. A full-suite live run during the audit pass found **5 real regressions introduced by this commit**, none caught at the time because they live outside the files/tests touched directly:
- `tests/test_iter129_chat_latency_budget.py::test_persona_under_budget` and `tests/test_persona_loc_guardrail.py::test_persona_under_hard_budget` — `AUREM_CTO_PERSONA` is now 22,627 chars, over the 22,000-char budget (the new ROLLBACK/REVERT/UNDO guidance section pushed it over).
- `tests/test_iter85_verified_paths.py::test_mutation_verbs_list_is_sharp_27_no_conversational_drift` — `MessageBubble.jsx`'s `MUTATION_VERBS` now has 31 verbs, not the locked 27 (added revert/rollback/undo/restore).
- `tests/test_session5_item2_orchestrator_silent_catch_lock.py::test_orchestrator_silent_catch_lines_are_locked` + `::test_locked_sites_still_call_hooks` — the new persona text shifted every downstream line number in `orchestrator.py`, breaking a locked-line-number snapshot test.

None of these were fixed in the audit pass (analysis-only, no code changes allowed that pass) — see `memory/RECON-LEDGER.md` row P0-4 for status and next action.

**NOT YET founder-confirmed**: no real end-to-end GitHub ship+revert was performed against a live repo (this Preview pod has no CTO project with a currently-working GitHub App install — `funnel-repro`'s install drifted/revoked per test_credentials.md). Founder should re-run their exact original repro on production (ship a trivial README edit, then ask ORA to roll it back) to get final live confirmation. The original 5 named production task failures (orchestrator.py x2, payments.py, 2 test files) remain unconfirmed against this specific fix — no real payloads were available to compare before/after.

## 2026-08-28 (continuation) — Loop N: rollback-PR risk memo, sign-in diagnosis, graph cap config, lint engine fix, Track B dry-run

**Scope note**: P0 hotfix from the prior loop stays OPEN — founder is running the production repro in parallel. Nothing here re-touches or re-verifies that fix.

1. **R10 — Rollback-on-PR risk memo** (`/app/memory/R10-ROLLOBACK-PR-GAP.md`, analysis only, no code changed): **Verdict: NOT SAFE as-is.** Root gap — the stored ship commit SHA is captured once, pre-merge, on the throwaway branch, and is never updated to the real `merge_commit_sha` after the PR merges (the merge webhook only updates a different collection's `pr_status` field). Concrete failure modes: (HIGH) a network blip on the live PR-state check fails closed to "not merged," causing rollback to silently no-op (close+retract a no-op on an already-merged PR) while reporting `"rollback_status":"done"` to the user — a false success. (HIGH) squash/rebase merges produce a diff/SHA that differs from the stale stored SHA, so even the "merged, fall through to revert" path has never actually reverted a squash/rebase-merged result correctly. `ship_via_pr` stays Preview-only/flag-OFF-in-prod per this memo; does not clear the R9 gate.
2. **R11 — Dead sign-in diagnosis** (`/app/memory/R11-SIGNIN-DIAGNOSIS.md`): confirmed via the app's own `/api/aurem-dev/version` endpoint that production (`02bdea1b0064`) and this pod (same lineage, HEAD) are running identical source — ruled out stale/different commit. The Sign-in button itself is a plain internal `<Link to="/login">` with zero env-sensitive code path (stated plainly, not speculated). Top ranked hypothesis: a service worker (`main.jsx` registers `/sw.js` on every load) stuck on an older cached version in the founder's own long-lived browser profile — exact DevTools check provided. Not yet founder-confirmed.
3. **Graph file cap → configurable**: `services/graph_builder.py`'s `MAX_FILES` is now `int(os.environ.get("GRAPH_MAX_FILES", "600"))` (was a hardcoded 200). Benchmarked against this pod's own ~1,627 real code files (comparable order of magnitude to the founder's reported 1,925+ file repo) — see `/app/e2e-proof/R10/graph_cap_benchmark.py`. The regex-extraction/memory half is trivial at any of these sizes (1800 files: 0.35s, ~4MB peak). The real cost driver is the network-bound file-fetch step (10 files/batch, 0.05s inter-batch sleep, 1 GitHub API call per file): 600 files ≈ 15-27s wall / 600 API calls; 1800 files ≈ 45-81s wall / 1,800 API calls. 600 is a moderate, safe default; 3x that (1800) is a real wall-clock + GitHub-rate-limit cost, not a memory problem. Updated the test that pinned `MAX_FILES == 200` to assert 600 + verify env-override actually works.
4. **ESLint engine error fixed at the source**: added `frontend/eslint.config.js` (minimal ESLint v9 flat config). Confirmed root cause: no `eslint.config.js` existed anywhere in the repo, so bare `eslint .` crashed with "couldn't find eslint.config.js" (an engine-level crash, not a lint result) — this is what the pre-completion checker was hitting. With the new config, `eslint .` now runs and returns real lint output (52 problems) instead of crashing. Did NOT install `eslint`/`eslint-plugin-react*` as local devDependencies — attempted this, but it breaks `@eslint/config-array`'s bundled `minimatch` against this repo's existing `resolutions.brace-expansion` pin in `package.json` ("expand is not a function"); reverted that attempt cleanly (`package.json`/`yarn.lock` confirmed back to original, `node_modules` reconciled via `yarn install`). `oxlint` (already installed globally, zero-config) remains this repo's actual clean-running linter — 166 warnings, 0 errors, 71ms.
