...
## STACK
FastAPI on port 8001 (supervisor-managed). Entry point: `backend/main.py` (all 55 `include_router` calls). Request flow: **Router → Core (Parliament) → Services → External APIs**.
...
## ROUTERS — `backend/routers/` (55 files)
| Category | Files |
|---|---|
| Auth & Identity | `auth.py`, `oauth.py`, `github_oauth.py`, `mfa.py`, `onboarding.py` |
| Chat / AI | `chat.py`, `chat_commits.py`, `loop.py`, `diagram.py`, `thinking_hints.py`, `advisor_context.py` |
| ORA Chat (founder-only) | `ora_chat.py` — SSE streaming `/message`, `/slash`, `/pin-login`, sessions, deep-research, codebase awareness (Iter 212m-245+) |
| Scanning | `codebase_health.py`, `security_scan.py`, `lint_preview.py`, `vanguard_ci.py`, `harden.py` |
| Fixing | `fix_pipeline.py` (SSE bulk fix, quota-enforced via `scan_fix_quota.py`), `findings.py` |
...
| Personal Track (T0-T4) | `personal_track.py`, `scaffold.py`, `supabase.py`, `managed_db.py` |
...
| QA / Findings | `qa_probe.py` (Iter 212m-190 Promptfoo internal QA) |
| Misc | `automations.py`, `support.py`, `upload.py`, `vault.py`, `version.py` |
