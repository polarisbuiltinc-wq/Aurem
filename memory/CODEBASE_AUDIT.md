...
| 3. Production founder password rotation (the actual fix for the leak) | ✅ **DONE — founder confirmed 2026-08-19** | Founder rotated production password via the self-service Forgot Password flow (built + browser-verified earlier this session). Old leaked passwords now dead/inactive. `rotate_password.py` (manual DB script) ended up not being needed — self-service flow was sufficient once it existed. |
