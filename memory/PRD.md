...
- **Session 2 · Redis rate-limiter (multi-pod fix)** — Redis diagnostic on prod confirmed root cause: `diag.host=127.0.0.1:6379`, `last_error=ConnectionError: Error 111 connecting to 127.0.0.1:6379. Connection refused.` Emergent's platform has no per-pod Redis. **DECISION-POINT OPEN** (founder ruling required): Option A ask Emergent Support for managed Redis, Option B Upstash external ($0/mo at today's scale, $1.50 at 100 devs, $75 at 5000 devs), Option C tightened per-endpoint stopgap ceilings (documented as TEMPORARY MITIGATION, NOT THE REAL FIX). Founder's stated preference: recommend A+C parallel.
...
- **Session 2.7 · REMAINING WORK for fresh session (A+B+D+F):** Ordered by founder in one bundle but split due to context budget. HAND-OFF SCOPE:
  - **Fix A · Buttonify `/image` in ORA output** — `OraDirect.jsx` needs regex-detect on ORA's markdown output for `` `/image ...` `` inline-code blocks and render a "Generate this image" button that fires `POST /image-generate` directly. Also wire similar buttons for `/read`, `/find`, `/defs`, `/repo-tree`, `/loop-stats`. Reuse existing endpoints — no new backend routes.
  - **Fix B · Slash-command whitelist in hallucination classifier** — `services/ora_chat/hallucination_classifier.py` or `adversarial_review.py`. Whitelist: the 13 KNOWN_COMMANDS from `safety.py` + `/image`, `/help`. Skip fabrication warning for any `/token` matching whitelist.
  - **Fix D · Sonar timeout increase + retry** — locate the Sonar / web-search client (grep for "Sources didn't respond in time"). Bump timeout 8s → 15s, add one retry-on-timeout, log Sentry warning when Sonar >5s.
  - **Fix F · Vision-upload redaction** — critical security fix. Before feeding extracted-text from `/upload` (or vision path) to the LLM context, run a redaction pass that strips patterns: `<PASSWORD>`, `<TOKEN>`, `test_credentials.md` content, email+password pairs, `Bearer <...>`, `sk-...` API-key shapes. Add pytest with synthetic credential-laced screenshot. **PROD INCIDENT EVIDENCE**: 2026-02-08 transcript showed founder's uploaded screenshot leaked test_credentials.md verbatim into ORA extraction.
  - All 4 must ship end-to-end with pytest + no mocks. One deploy. Use fresh session with clean context — this session is out of budget.
