...
**Fixed:**
1. **Demo-audio MP3 eager-fetch (real bandwidth waste, ~59 KiB)** — `WalkthroughPlayer.jsx`'s audio-sync effect called `a.src=...; a.load()` unconditionally on mount/step-change, regardless of mute state — so the homepage embed (`Landing.jsx` line 993) fetched `step-1.mp3`/`step-2.mp3` the instant it mounted, even though unmuted-autoplay-with-sound is blocked by browser policy anyway (so `.play()` was silently failing already — no functional loss). Fixed: default `muted` state flipped `false`→`true` (standard "muted autoplay, click to unmute" pattern), and the effect now skips `.src`/`.load()` entirely while muted — audio only fetches once the user explicitly clicks Unmute.
2. **3rd-party analytics (Meta Pixel + Google Ads gtag) deferred further** — both were already `requestIdleCallback`-deferred (part-3/prior rounds) with a flat 2.5-3s timeout. Added a shared `window.__auremDeferThirdParty(fn, idleTimeoutMs)` helper in `index.html`: fires on the FIRST of (user interaction: scroll/pointerdown/touchstart/keydown/mousemove) OR idle-callback with a 4.5s timeout fallback — real visitors almost always trigger the interaction path within ~1s, pushing the ~650 KiB combined JS parse/eval further past LCP/TBT measurement without dropping PageView/conversion tracking for lurkers (idle-timeout fallback still fires it eventually).
3. **Not touched, documented reasoning:** route-level code-splitting — ALREADY implemented since Iter 123g/358 (verified via `App.jsx` read: only `Landing` is eager-imported, every other page incl. Login/Signup is `React.lazy`). Legacy JS (23 KiB) — no `@vitejs/plugin-legacy`/`core-js` in `package.json`, Vite's default esbuild target is already modern-only; likely third-party-controlled (fbevents.js itself), not worth chasing for 23 KiB. Cache-lifetime (424 KiB) — hosting/CDN layer, no code-level lever found.

**Testing:** `testing_agent`, frontend-only, `/app/test_reports/iteration_perf_muted_default_2026_01.json` — 7/7 passed live on preview: mute-by-default confirmed, zero `/demo-audio/*` network requests while muted (even after scrolling embed into view), unmute click correctly triggers the MP3 fetch (audio path still works end-to-end), visual playback/step-dots/captions unaffected by mute state, `window.fbq`/`window.gtag`/`window.dataLayer` all confirmed to still initialize within ~5.5s + interaction simulation. `retest_needed: false`.

**Flagged by testing_agent, NOT fixed this round (nit + a finding needing follow-up look):** (a) unmute triggers step-1 + step-2 + step-3 MP3 prefetch within ~4s (likely intentional next-step preload, ~30 KiB extra per unmuted session — minor, only affects users who opt into sound); (b) testing_agent's screenshot showed "3 native `<audio controls>` elements" elsewhere on the landing page below the walkthrough embed — grepped the codebase, found ZERO other `<audio>` tags anywhere in `components/`/`pages/` besides `WalkthroughPlayer.jsx` (whose own `<audio>` is `display:none`, no `controls` attr) — root cause of what testing_agent saw is UNRESOLVED/unexplained, could be a screenshot mis-read or a dynamically-injected element; needs a direct screenshot re-look next round before acting on it.

**PROVENANCE: preview-verified only (testing_agent, live preview URL).** Founder must redeploy to production and re-run PageSpeed to confirm the updated mobile score; the report crawled this round may have been stale (predated the part-3 deploy), so an accurate before/after comparison is still pending founder's own fresh test.
...
**ORA Self-Repair Log (`/admin/self-bugs`):** loaded fine on preview via screenshot — table renders, 136x-seen `dead_end_leak` entries visible, filters work. It's a READ-ONLY ledger (no actual "fix now" action exists — never has). No bug found here on preview; if founder meant something is broken on production specifically, or expected an active repair button that doesn't exist, need their follow-up.

**Codebase Health — REAL BUG found and fixed, self-reproduced via curl (not guessed):**
- `POST /codebase-health/scan` for a project with a stale/missing GitHub App link crashed with an **unhandled** `GithubAppAuthError` from `get_repo_token(proj)` (`routers/codebase_health.py` line ~282) — there was ZERO try/except around that call, unlike the very next block (`_build_text_cache`) which already had one. The unhandled exception propagated through FastAPI's middleware stack and surfaced to the client as a mangled `"client disconnected or upstream error"` instead of any usable message. Since the scan never got far enough to compute anything, `GET /codebase-health/last` stayed `score: None` forever — exactly matching "score show nahi ho raha" (score never shows).
- This is a genuine robustness gap, not fixture-specific: `GithubAppAuthError` can legitimately fire in production too (transient GitHub 401/403 on a specific installation's token mint, a revoked/suspended install, etc.) even when the App is overall healthy — `services/pat_vault.py`'s own comments document this exact "false alarm mid-ship" class.
- **Fix:** wrapped `get_repo_token(proj)` in `routers/codebase_health.py::scan` with `try/except GithubAppAuthError`, mapping `.code` to a clean HTTPException — 400 (user must reconnect) for `app_installation_missing`/`app_installation_revoked`, 502 (transient, retry) for `github_unreachable`/`github_rejected`. Re-tested the exact same failing curl call after the fix: now returns a clean `400` with the real actionable message instead of the crash.
- Tests: `tests/test_codebase_health_scan_auth_error_2026_09_09.py` (2/2 pass — source-lock on the try/except + functional mock proving the clean 400). Regression: `test_phase2c_codebase_health_router.py` + `test_iter212m216_gh_error_propagation.py` + `test_iter212m217_scan_rate_limit_shape.py` + `test_iter363_phase3b_github_app_dispatch.py` — 53 passed, 5 pre-existing failures confirmed identical before/after via git-stash A/B (unrelated: `TestLastScan`/`get_repo_token_dispatch_*` fixture issues, not touched by this fix).

**PROVENANCE: preview self-reproduced + fixed, NOT production-confirmed.** This is a plausible but not 100%-confirmed match for the founder's exact production complaint (I could not get their exact repro/screenshot — they said "proceed with best judgment" after being asked). Founder should re-run a Codebase Health scan on production and confirm the score now populates; if it still doesn't, the cause is elsewhere (need a screenshot/exact error at that point).

**Fabrication bug (a1b2c3d) + dead-end/silence bug — STILL UNRESOLVED, blocked on exact repro text.** Verified via git history that my 2026-09-09 fabrication fix (system-prompt guard in `ora_context.py` + `output_guard.py` strip pattern) WAS present in the build the founder retested (b7a23f34) — yet the exact same fabrication reportedly still occurred. Root cause not yet found — candidates identified this round: (a) the model may not obey the system-prompt guard reliably on the "Swift" (cheap) model tier, (b) the actual live text format may not exactly match the narrow bracket regex in `output_guard.py`, (c) one of several OTHER independent system-prompt-construction paths in `routers/chat/worker.py` (`_mode_b`→`mode_b_council.py`, `_mode_f`→`mode_f_engage.py`, `_ora_agent_cascade`'s own `ora_system`, `loop_engine.py`'s separate `sys_msg` for Loop's plan phase) never call `render_ora_boundary_prompt()` at all, so my guard never reaches them — though `strip_machinery_leak` IS applied universally after `run_worker()` regardless of which mode produced the text, so an output-guard miss still points to a literal text-format mismatch rather than a bypass. **Asked founder for the exact literal transcript twice; not yet provided.** Do NOT attempt a third blind fix — get the real text first.


## 2026-09-09 (deploy debug round) — Real deploy-blocking bug found + fixed: `_sh()` git calls were blocking the event loop, causing K8s health-probe timeouts mid-deploy

**Founder's deploy logs (2 rounds):** round 1 showed a MongoDB Atlas `ServerSelectionTimeoutError`/`ReplicaSetNoPrimary` — `deployment_agent` scan + direct code read of `main.py`'s Mongo client (Iter 335 hardening: `minPoolSize=0`, sane timeouts, `retryWrites=True`, already correct) concluded this was an Atlas-side network blip (IP allowlist / cluster hiccup), not a code defect — reported as such, no code change (would have been guessing at a non-existent bug).

**Round 2 — the real, code-level, deploy-failing bug:** logs showed many healthy `/health` 200s, THEN a burst of `nginx [error] upstream timed out ... GET /health ... upstream: "http://127.0.0.1:8001/health"`. `/health` (bare path → `healthz_root()` in `main.py`) is a trivial `return {"ok": True}` with zero I/O — nginx/K8s's OWN health probe timing out against it means the whole backend event loop was blocked/wedged for many seconds, not a bug in that route itself.

**Root cause, confirmed by direct code read:** `services/cto_projects_helpers.py::_sh()` is a bare synchronous `subprocess.run()` git helper (clone up to 120s, push up to 90s). It was called with ZERO `asyncio.to_thread` offload from `_run_task_with_git` (`routers/cto_projects/worker_git.py`, 8 call sites) and `_run_rollback_with_git` (`routers/cto_projects/rollback.py`, 6 call sites) — both run via FastAPI `BackgroundTasks` on the SAME event loop as every other request. A real git clone/push during a ship or rollback blocks that loop for its full duration, starving every concurrent request including the trivial `/health` probe — exactly the observed nginx timeout burst, which makes K8s mark the pod unhealthy and fail/restart the deploy. **This was already flagged and explicitly deferred** in the 2026-09-02 "Event-loop-blocking FIX" round (PRD: "`_sh()` ... 15+ call sites ... FLAGGED, deferred, NOT fixed this round") — this round closes it.

**Fix:** every `_pkg._sh(...)` call site in both files is now `await asyncio.to_thread(_pkg._sh, ...)` — same pattern already used for `write_repo_file`'s syntax gate and `deploy_readiness.py`. Zero logic change (same commands, same timeouts, same return-value handling), pure async-offload. `rollback.py` needed `import asyncio` added (was missing); `worker_git.py` already had it.

**Tests:** `tests/test_git_worker_event_loop_offload_2026_09_09.py` (3/3 pass) — source-lock guard (no bare non-offloaded `_pkg._sh(` call remains in either file) + functional proof (a slow `_sh`-shaped subprocess call via `asyncio.to_thread` doesn't starve a concurrent heartbeat). Regression: `test_cto_worker_safety_pipeline_2026_09_08.py` + `test_iter_cto_projects_worker_coverage_2026_08.py` + `test_sensitive_path_guard_2026_08_27.py` (20/20) + `test_phase2c_cto_projects_router*.py`/`test_iter388h/n_*`/`test_checkpoint_resume_2026_08_27.py` (210/211 — the 1 failure, `TestSubmitTask::test_rate_limited`, confirmed pre-existing/unrelated via `git stash` A/B on just these 2 files). `python -m py_compile` clean on both files.

**PROVENANCE: preview-verified only.** This is a real, plausible root cause for a genuine deploy-time failure (not a guess like the Atlas theory) — but it has NOT been confirmed by an actual successful production deploy yet. Founder should redeploy and confirm no more `/health` upstream-timeout bursts appear in the new deploy logs.


## 2026-09-09 — Fake tool-call/tool-result fabrication (founder live-caught in PRODUCTION) — guard shipped, PREVIEW-verified only

**Founder's exact prod repro:** told the model to "check", it wrote `[Tool note: Checking production status via site_ops.check_production_publishing_status()...]` then `[Tool result: ... update ID: a1b2c3d ...]` — a fabricated tool call AND its fabricated result (the SAME generic fake hex ID repeated across unrelated requests proves it's a template, not a real check). Founder's own root-cause read: brand-critical fabrication class, worse than any prior bug in this thread ("we verify, we don't hallucinate" is the core promise).

**Ruled out first (per founder instruction):** GitHub App credential theory is DEAD — founder confirmed `key_state=VALID`, 45 installations, real token mint, in their own prod browser. Confirmed via code read: `services/orchestrator.py` builds the tool catalog from the REAL upstream `list_tools()` + `LOCAL_TOOL_SPECS` (no hard-coded promised-but-unwired tool list) — so this is the model narrating a wholly invented tool name/result in prose, not a catalog mismatch.

**Fix shipped (both layers, per founder's fix principle):**
- PRIMARY — `services/ora_context.py`: added an explicit anti-fabrication rule to ALL 3 ORA ABSOLUTE BOUNDARY variants (repo-scoped rule 7, no-repo rule 5, founder-pod-debug rule 6): may only report a tool's output if it actually called that tool; never write `[Tool note: ...]`/`[Tool result: ...]`-shaped text or invent an ID/timestamp/status; say "I have not checked this yet — I can check now" instead.
- BEST-EFFORT NET — `services/output_guard.py::_UNIVERSAL_LEAK_PATTERNS`: new regex strips a `[Tool note: ...]`/`[Tool result: ...]` bracket block (combo matched first so a paired fabrication gets ONE disclaimer, not two), replacing with an honest "I have not actually run that check yet — I can check now if you'd like." Already wired into BOTH live paths (`routers/chat/turn.py` + `routers/chat/stream.py`) with zero extra plumbing — both already call `strip_machinery_leak(..., universal_only=...)` unconditionally for every user. Deliberately NOT a general detector (founder: don't over-engineer) — narrow bracket pattern only.
- Tests: `backend/tests/test_fake_tool_call_narration_guard_2026_09_09.py` (6/6 pass) — reproduces founder's exact transcript, asserts single disclaimer for the paired case, asserts normal prose mentioning "tool"/"result" is untouched (false-positive guard), asserts all 3 boundary-prompt variants carry the new rule.
- Regression: `test_narration_leak_strip_2026_09_02.py` + `test_m3_output_guard_named_file_fix_2026_08_30.py` still 100% pass (14/14 combined with the new file). Confirmed via `git stash` A/B that the 18 pre-existing failures in `test_iter212m169/170_*`/`test_iter388t_bug20_*` (DB-mock related) are identical before/after this change — not caused by it.

**PROVENANCE: preview-only.** This has NOT been deployed or re-tested in production. Founder action needed: deploy this build to production, then re-run the exact repro ("check production status"/similar). If the fake `a1b2c3d`-style block is gone, this fix landed; if it still appears, the guard needs a second look (paste the new prod transcript).

**Not touched, per explicit founder instruction:** ship-path (Wave-2/PR flow) — untouched this round.

**Also confirmed this round (no code change needed):** D1 Personal Track hide — ALREADY implemented and locked by an existing test (`Settings.track_switcher_hidden.test.js`); `TRACK_SWITCHER_ENABLED = false` in `frontend/src/pages/Settings.jsx`, component/routes intact, not deleted.

**Also this round — GitHub App production diagnostic path documented (dead-end, but worth keeping):** confirmed GitHub App config is DB-stored (`admin_settings._id="github_app_config"`), not an env var — `GET /api/aurem-dev/admin/github-app-config` (key_state) and `GET /api/aurem-dev/admin/github-app-diagnostics` (full probe) are the founder-runnable, admin-JWT-gated read-only endpoints for any future App-health question. Founder ran these directly in prod browser — healthy, not the cause of this round's bug.


## 2026-09-08 (round 4) — CRITICAL SCOPE CLARIFICATION: preview vs production + secret-detection evidence + PAT test rewrite

**Preview vs production (confirmed via support_agent, this is now the standing disclosure for every round):** the agent has **NO access to or visibility into the founder's production environment** — no logs, no ability to curl/test the live prod URL. Every "green"/"verified" claim in this entire chain (audit, 4 risks, Decision 1/2) is **preview-only**. Code does NOT auto-deploy — reaching production requires the founder to explicitly click Deploy → "Deploy Now" in the Emergent UI (~10-15 min), separate from this Preview. Once deployed, the founder can view/manage the app, env vars, domains, and rollback from the Home tab — the agent still has no log/monitoring access to it. **Going forward, every fix report will explicitly say "verified in PREVIEW" and will not claim production status unless the founder confirms it themselves after deploying.**

- **Secret-detection evidence (Risk #3), requested 3x now — delivered:** could not produce a git-history before/after diff (the redacted placeholder was already present in this repo's earliest tracked commit for these files — the scrubbing happened before tracked history began). Produced DIRECT, reproducible evidence instead: fed a fresh, realistic AKIA/AIza-shaped secret built purely in-memory (never touched disk/git, so it cannot itself have been scrubbed) through the unmodified live scanner code — **detected correctly** (`secret_aws_access_key`/`aws_access_key_id`/`gcp_api_key`, severity=critical). Fed the OLD placeholder text (`"***REDACTED_AWS_KEY***"`) through the same unmodified scanner — correctly NOT detected, because it genuinely isn't key-shaped text (not because the regex is broken). Also confirmed `_scan_text`/`scan_bug_hunt` are wired into the LIVE user-repo-facing scan path (`routers/security_scan.py`, `services/full_scan_orchestrator.py`, the Loop's own pre-ship Vanguard scan) — not orphaned/test-only code. Marked all 4 affected tests `@pytest.mark.known_fixture_scrubbed` (new marker, documentary only, NOT excluded from the default run — they pass now) with the full evidence transcript embedded as a code comment, so this doesn't need re-litigating every round.
- **PAT test → GitHub App rewrite (test-only, NOT a PAT re-enable — AUREM uses GitHub App auth exclusively):** rewrote `tests/test_aurem_p0_bugs.py`'s `created_project` fixture to connect via a real `installation_id` (matching the same pattern `test_github_app_project_add.py` and `test_h3_loop_repo_pin_2026_08_30.py` already use), fixed the PATCH test that used to also try updating `github_token` (now a dedicated test asserting it 400s), and swept a stale leftover row from a DIFFERENT test file's own non-idempotent fixture that was blocking with a real 409. **Found, while doing this: this pod's GitHub App installation token is currently being REJECTED BY GITHUB ITSELF** ("the installation may have been suspended") — confirmed via direct backend log inspection, this is the SAME pre-existing GitHub App credential staleness issue already documented earlier in this project's history, not something introduced by this rewrite or fixable by a test change. 9 of this file's tests remain blocked on that until the founder refreshes the GitHub App's credentials in Admin settings for this pod. This is itself supporting evidence for the founder's exact "preview isn't fully healthy either" concern.


## 2026-09-08 (round 3) — Founder decisions executed: Anti-Fabrication ported into v2, /ora hardened

**Decision 1 — DONE, tested green.** Ported the Anti-Fabrication Regen safety mechanism into `services/ora_chat_v2/engine.py::run_turn` (reused the existing `services.ora_chat.grounding_check.run_post_response_check` — no new detection logic invented). When `ORA_REGEN_ON_FABRICATION=1` and the drafted reply cites a file that doesn't exist in the real codebase index, ONE corrective regen call fires (via `llm_client.stream_chat(tools=None, ...)`) before any delta is streamed to the client; the final SSE event now carries `ungrounded: []`/`[...]`. Rewrote `tests/test_anti_fabrication_regen_admin.py` to mock `services.ora_chat_v2.llm_client.stream_chat` (the old `routers.ora_chat.stream_call`/`one_shot` symbols it patched no longer exist post-v2-migration) — drives the real `/ora-chat/message` endpoint end-to-end, proves the fabricated draft never reaches the client and the corrective draft is what streams + persists. **The anti-fabrication guarantee is live in v2.** 2/2 tests pass; confirmed no regression to the 6 pre-existing (baseline) ora_chat_v2 test failures (same failure signature before/after, isolated repro confirms pre-existing).
**Decision 2 — DONE, tested green (backend 100% + frontend 100% via 2 testing_agent rounds).** `/ora-chat/pin-login` hardened: (1) new `ORA_ALLOWED_IPS` env allowlist checked BEFORE the PIN (403 `ip_not_allowed` for non-listed IPs) — left UNSET in this pod (fail-open, unchanged behavior) since the agent can't verify the founder's real IP; founder sets it when ready. (2) `ORA_QUICK_PIN` rotated from 4-digit `7668` to 12-char `FefzrqxpCRQg` (see test_credentials.md — founder must rotate prod's PIN too). (3) 5/hour/IP+account lockout kept as-is. (4) every successful mint now logged to Mongo `ora_pin_mint_log` (ip+identity+timestamp). New test `tests/test_ora_ip_allowlist_2026_09_08.py` (3/3, includes named `test_t_ora_ip_allowlist_blocks`). **Caught + fixed a self-inflicted regression during this same round**: rotating the PIN to alphanumeric broke the frontend `PinPad` (`frontend/src/pages/OraDirect.jsx`), which had a hardcoded `PIN_LENGTH=4` digit-only keypad — testing_agent caught this as CRITICAL before it shipped un-verified; rewrote `PinPad` to a plain masked text input + Unlock button (removed the now-dead `PadKey` component), re-verified 100% pass including the exact previously-broken flow. Per-person 2FA for `/ora` explicitly deferred (documented in ROADMAP.md, pre-scale — not needed for a solo-founder team yet).


## 2026-09-08 (audit follow-up round) — Fixed the 4 audit-flagged risks + /ora decision surfaced

**Context:** founder ran a comprehensive read-only codebase audit, then asked for exactly 4 fixes (in order) plus one decision item. All 4 done and testing_agent-verified (30/30 backend, 28/28 frontend, no regressions — `/app/test_reports/iteration_risk1_risk2_verify_2026_01.json`).

- **Risk #1 FIXED — ShipPendingCard safety regression.** Legacy `ShipConfirmModal` showed a Vanguard preflight badge before Ship; Loop-mode `ShipPendingCard` had none. Backend: `services/loop_engine.py` now surfaces `vanguard_verdict` (`{ran, critical, high, total}`) at the ship-pause point, reusing the ALREADY-computed Phase-4 Vanguard scan (zero extra cost) — written into both `self.context["ship_pending"]` and the SSE `awaiting_ship` event payload. Frontend: new `VanguardPill` in `ShipPendingCard.jsx` (`data-testid="ship-vanguard-pill"`) renders clean/flagged/unknown, alongside the existing diff chips + integrity pill, always in the same render as the (enabled) Ship button — no blind ships. Also fixed `frontend/src/lib/shipPendingMappers.js`'s two mappers to pass `vanguard_verdict` through (this exact class of bug — a mapper silently dropping a verdict field — had already bitten `integrity_verdict` once before). Named test `t_ship_card_shows_diff_and_verdict` added in 3 files (component + both mapper/integration test suites).
- **Risk #2 FIXED — auth surface.** Root-caused `test_revoke_all_sessions_kills_every_token_for_user` as **test-isolation flakiness, NOT a real revocation gap**: it hits the real backend over HTTP and shares a process-wide `login-ip:<ip>` rate-limit bucket with every OTHER test file that also calls real `/auth/login` in a full-suite run — live-confirmed via curl (6 rapid logins → 429 on the 6th). Fixed by giving the test its own synthetic `X-Forwarded-For` IP; added named regression test `test_t_revoke_all_sessions_actually_kills_all` that deliberately exhausts an unrelated IP's bucket AND proves the real flow still works cleanly on an isolated IP. Also fixed a stale test string (`test_login_burst_rate_limiter_exists_and_binds` checked the old sync `check_rate_limit(` name, code was refactored to `check_rate_limit_async`). Added REAL missing coverage: `test_iter337_auth_me_no_secrets.py::test_me_live_response_strips_pending_mfa_enrollment_secrets` (the 2026-09-08 pending-MFA-fields leak fix had ZERO test coverage until now) and `test_iter212m20_admin_2fa_and_tabbar.py::test_live_two_step_2fa_login_issues_session_and_leaks_nothing` (full live round-trip with a real pyotp-generated TOTP code — prior coverage was source-string-only, never actually drove the endpoint).
- **Risk #3 FIXED — secret-detection test regression was a TEST-FIXTURE ARTIFACT, not a live scanner bug.** Confirmed: the AWS/GCP-shaped literal fixtures in 3 test files had been auto-redacted to literal `"***REDACTED_AWS_KEY***"`/`"***REDACTED_GOOGLE_KEY***"` text by GitHub push-protection (or an equivalent scrubber) after being committed — those placeholders can never match the (unchanged, correct) detection regexes. Fixed by rebuilding the fake secrets from string-concatenation fragments so no single literal token is secret-shaped in the diff. All 3 tests pass.
- **Risk #4 — individual baseline triage started (SLA added).** Added a 14-day individual-triage SLA to `memory/ROADMAP.md` P0.5. This round: fixed `test_aurem_p0_bugs.py`'s `created_project` fixture (found **11 accumulated stale DB rows** from a non-idempotent fixture using a fixed repo slug — cleaned up + fixed to self-heal), which then exposed a SECOND, deeper, real issue: the file still tries to connect via PAT (`github_token`), which was **removed entirely per a June 2026 founder directive** — ticketed as a follow-up (needs a mock-based rewrite or retirement, not a quick patch). **Found a real, confirmed regression** (not fixture drift): `test_anti_fabrication_regen_admin.py` fails because `routers/ora_chat.py::send_message` no longer calls `one_shot`/`stream_call` at all — it now delegates to `services/ora_chat_v2/engine.py::run_turn`, which has **zero** grounding-check/anti-fabrication-regen logic. The Anti-Fabrication Regen safety feature (shipped 2026-08-19) is currently dead code in the live admin ORA chat. **Did not patch the test to hide this** — ticketed in ROADMAP.md P0.5 as HIGH, needs a founder decision (port the logic into v2, or formally retire the feature).
- **`/ora` privilege-path finding corrected and escalated (F18 note in ROADMAP.md).** Founder's framing was "any logged-in user + shared PIN" — live-verified this round it's actually **worse**: `POST /ora-chat/pin-login` takes no Authorization header at all; it's a fully public, unauthenticated endpoint (the `<PrivateRoute>` on the frontend `/ora` route only gates the page render, not the API). Confirmed via curl with no auth header. This is a deliberate, documented design choice (one-tap founder bookmark access), not an accidental bug — flagged for founder ruling on whether to harden it (longer PIN / 2nd factor / IP allowlist) or keep as an accepted trade-off. Not changed this round per the audit's explicit "decide, don't fix" instruction for this item.
...
**Built:**
- `backend/services/actions/pending_action.py` (new) — full PendingAction lifecycle in its OWN dedicated Mongo collection (`pending_actions`, deliberately NOT piggy-backed on `chat_sessions`): `PROPOSED -> validate concrete payload -> AWAITING_CONFIRM -> confirm -> EXECUTING -> apply -> EXECUTED -> read-back verify -> VERIFIED / APPLIED_FAILED`, or `CANCELLED` via explicit cancel or TTL (20 min) expiry. Deterministic pre-LLM classifier (`classify_confirm_intent`) resolves `confirm` / `cancel` / `new_request` (plus bare-number disambiguation replies) without ever calling the LLM. At-most-one-actionable-per-session is enforced via disambiguation (numbered list + numeric reply), never silent pick or stacking. Idempotent: re-confirming an action already VERIFIED/APPLIED_FAILED within 5 min echoes the SAME stored result, never re-executes (no double write, no double Stripe charge attempt).
- `backend/services/commit_boundary.py` (new) — the ONE glue module both `/chat/send` and `/chat/stream` call. Gated by `COMMIT_BOUNDARY_ENABLED` (backend/.env, default `true`): ON routes to the new deterministic machine; OFF falls back verbatim to the prior 2026-09-04 `services/confirm_execution.py` guard-based behavior (UNCHANGED, kept intact as the rollback lever — its own 30-test suite still fully passes). Also carries `strip_false_confirm_promise()` — a deterministic guard stripping a dangling "ready when you approve?" question after a real `aurem-handoff` fence (the fence + Approve button IS the ask; a trailing text question is always misleading under this architecture), wired into the single shared `apply_output_guards()` in `chat_helpers.py`.
- CBR-1 enforcement (prose cannot create an action): `propose_from_turn()` (called once per turn at persist-time, replacing the old inline register block) only creates an AWAITING_CONFIRM doc when `extract_deterministic_edit()` finds a clean `(path, old_value, new_value)` triple in the reply AND that exact `old_value` is validated as uniquely present in the REAL live file (read via `read_repo_file`) — anything vague, or lacking repo context to validate against, is persisted as PROPOSED then immediately CANCELLED (traceable but never actionable). Complex multi-file `aurem-handoff` briefs stay button-only (the separate, already-tested task-queue engine) — text-confirm no longer attempts an LLM-replay fallback for those (the old `confirm_execution.py::_execute_code_fence_replay` risk is gone from the new path entirely).
- CBR-5/8 (verified-before-"Done"): every executor (`_execute_edit`, `_execute_upgrade`) performs an independent read-back after the side effect (re-read the file / re-fetch the `cto_payments` row) before returning success framing; a failed read-back is always `APPLIED_FAILED` with an honest message, never a false "Done".
- Upgrade executor reuses the EXISTING `routers/payments.py::create_checkout_session` unchanged — fixes the sibling bug where a confirmed upgrade offer returned a generic clarification instead of a real checkout URL.
- `backend/routers/chat.py` — both call sites (`/chat/send` ~line 616, `/chat/stream` ~line 2557) now call `commit_boundary.resolve_turn_start()` instead of `confirm_execution.maybe_execute_pending()` directly; `_persist_turn()` now threads `bin_ctx` through so persist-time proposal validation can read the real repo file.

**Tests:** `backend/tests/test_commit_boundary_2026_09_05.py` — 19 tests covering all 13 founder-named acceptance tests verbatim (`t_confirm_is_state_transition_not_model_turn`, `t_confirm_executes_concretized_payload_not_reproposal` — the CSS-class-survives-verbatim smoking gun, re-proven — `t_confirm_no_pending_is_honest_actionable`, `t_confirm_disambiguates_multiple`, `t_prose_cannot_create_action` x2, `t_confirm_always_resolves` x3, `t_factual_claim_not_shown_unverified`, `t_done_only_post_verify`, `t_turn_always_terminates`, `t_pending_persists_across_turns`, `t_confirm_idempotent`, `t_ttl_expiry_cancels`, `t_confirm_bounded_to_verified` — all pass). One assertion updated in `test_confirm_execution_2026_09_04.py` (call-site name changed, module itself untouched).

**Testing-agent verified** (`/app/test_reports/iteration_commit_boundary_2026_09_05.json`): 19/19 new + 30/30 fallback + 91/92 broader regression (the 1 miss is `test_happy_path_home_no_project`, a non-deterministic real-LLM-greeting-text baseline flake confirmed via git-stash A/B to pre-exist this round). Live preview curl confirmed the exact founder-mandated strings verbatim for zero-pending confirm and zero-pending cancel. `retest_needed: false`.

**Explicitly BLOCKED, not a code defect — reported, not fabricated as passing:** the founder's final acceptance gate (real UI + real writable repo terminal-state proof) could not run — this preview pod's only GitHub-App-connected fixture project (`funnel-repro` / `polarisbuiltinc-wq/ora-grounding`, installation 152797252) returns `github_rejected` / HTTP 401 minting the installation token (`GET /cto/projects/{id}/check-pat` -> `state:"missing"`). This is a pre-existing infra gap documented across multiple prior sessions (Loop-2, overnight T7), not introduced this round. Mechanism-level proof (verified read-back, literal substitution, no re-proposal) is fully covered by mocked-repo unit tests instead. **Founder action needed to close this specific gate**: reinstall/reauthorize the AUREM DevOps GitHub App on a disposable drill repo, or confirm a different working fixture, then re-run just the real-UI/repo terminal-state tests.

**No new deps. `COMMIT_BOUNDARY_ENABLED=true` in Preview only — founder deploys to production when ready; no prod flip by agent.**
...
---
**2026-09-09 — D1/D2/D3 founder-locked round (chip dismiss, real Unlock-Pro checkout, freemium 10-fixes/month). PREVIEW ONLY.**
- D1: `FirstScanCard.jsx` — the "your site looks in good shape" chip now has a dismiss X (`first-scan-clean-dismiss-btn`) AND auto-dismisses on the user's first chat message (`dismissOnFirstMessage` prop from `ChatPanel.jsx`, `messages.some(m=>m.role==='user')`). No fixed timer, per founder's explicit rejection of that.
- D2: Root cause was the backend already creating a real Stripe checkout session on upgrade-confirm but never surfacing it to the frontend (`result.meta` was silently dropped in both `/chat/send` and `/chat/stream` response shapes). Fixed: `turn.py`/`stream.py`/`worker.py` now include `meta` end-to-end; new `UnlockProCTA.jsx` renders a real "Unlock Pro →" button on `edit-tier-upgrade-offer`/`fix-quota-exceeded` bubbles (or any bubble with `meta.checkout_url`), opening checkout in a NEW TAB. `ModeLoopPill.jsx` now gates Pro/Maxx selection with `getUnlockedModesSync()` (new helper, mirrors `subscription_tiers.py` exactly) — a locked click shows the existing `UpgradePopup` (now exported from `ModeSelector.jsx`) instead of silently letting the backend clamp the mode.
- D3: Freemium "10 fix/ships per month, monthly reset" — extended the EXISTING quota system (`services/subscription_tiers.py` free.tasks_per_month=10 was already correct; `services/usage.py::assert_has_task_budget` reused as-is with updated message copy). Moved the enforcement point from "unconditionally at the top of every chat turn" (was blocking plain chat too — a real bug) to "only when result is None and tier=='agentic'" inside `resolve_pre_llm` (new step 3.5), plus a matching gate in `pending_action.py::execute_action` for the deterministic single-file edit/insert confirm path. Both fail OPEN on any non-402 error (fixed a mid-round regression where a 404 "user not found" for synthetic test users was wrongly treated as quota-exceeded — caught via test failures, narrowed to `status_code==402` only, re-verified 37/37 green).
- Testing: backend 37/37 green (`test_commit_boundary_2026_09_05.py` + `test_chat_pre_llm_dedup_2026_09_06.py` + `test_insert_edit_type_and_nonfenced_council_2026_09_09.py`). Live E2E: D2a (real Stripe checkout opens in new tab) and D2c (locked-mode upgrade popup) **fully verified live** on preview with a genuine free-tier test account. D1/D2b/D3 **code-verified only** — could not manufacture a live "clean scan" or "10 real completed fixes" state in preview; explicitly NOT claimed as prod-confirmed. Report: `/app/test_reports/iteration_d1_d2_d3_founder_locked_2026_09_09.json`.
- 3 P0s (ship-button, delete-confirm, 0:00 disable) — untouched this round, re-confirmed via code inspection only (not live re-tested, since testing_agent scoped effort to the new D1/D2/D3 surfaces).

---
**2026-09-09 — ORA Browser Diagnostic Tool + Founder Decisions Round (PREVIEW ONLY).** NEW `services/browser_tools.py`: real Playwright tool for ORA — navigate/click/fill/submit/screenshot/a11y_snapshot/console_errors/network_log/wait_for/back/close, ONE code-driven session per turn, semantic a11y snapshot as primary "sight", fresh profile + closes every turn, SSRF+scheme guard (blocks all RFC1918/loopback/link-local/metadata + file/data/ftp, public http/https only), reuses `loop_safety.acquire_loop_lock` so it can't race a Loop ship on the same project (`project_busy` if held), honest degrade if Chromium missing. Wired end-to-end into ORA's real chat tool loop (`local_tools.py` LOCAL_TOOLS+TOOL_SPECS, `tools_bridge.py` `_KNOWN_TOOLS`, `orchestrator.py` system-prompt instructions). 20/20 real non-mocked E2E tests pass (`test_browser_diagnostic_tool_2026_09_09.py`, real hits to example.com/httpbin.org). D1: AskAdvisor confirmed unchanged (comment-only ruling, 6/6 tests pass). D2: TrackSwitcher-hidden-for-regular-users found ALREADY implemented pre-existing (`TRACK_SWITCHER_ENABLED=false`), re-verified via vitest + live screenshot. Ship-meter 4 fields + "ORA by Aurem" naming found ALREADY implemented from earlier work, re-verified only. testing_agent: 85 passed/12 skipped backend + 5/5 frontend, 0 critical/minor code issues (`/app/test_reports/iteration_browser_diagnostic_tool_2026_09_09.json`). **KNOWN RISK, NOT PROD-CONFIRMED**: production Chromium binary availability for the deployed image is unverified (see `SUPPORT_TICKET_DRAFT_CHROMIUM.md`) — founder must validate post-deploy before calling this production-ready. Minor non-blocking gap: no dedicated unit test yet for the `project_busy` lock-contention branch.
