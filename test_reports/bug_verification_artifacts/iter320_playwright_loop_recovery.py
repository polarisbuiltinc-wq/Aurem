...
async def main():
    ...
    async with async_playwright() as p:
        ...
        try:
            await page.wait_for_function("document.body.innerText.includes('Plan taking longer than expected')", timeout=10000)
        ...
        # Extra deterministic guard for Iter 320: allow the chip's 10s
        # /loop/active poll to run after the mocked backend state has moved to
        # awaiting_confirmation. If the chip still displays PLANNING, this is
        # not just a transient stale-poll window; the visible label is mapping
        # phase=plan ahead of state=awaiting_confirmation.
        await page.wait_for_timeout(12000)
        txt_after_poll = await dump(page, "recovery_after_sse_after_poll")
        chip_after_poll = ""
        if await page.get_by_test_id("loop-status-chip").count() > 0:
            chip_after_poll = await page.get_by_test_id("loop-status-chip").inner_text(timeout=2000)
        if card_visible and "LOOP · PLANNING" in chip_after_poll and "Plan ready" in txt_after_poll:
            result.fail("PLAYWRIGHT-4 after poll", f"Chip still says planning after a full poll interval with approval UI visible: chip={chip_after_poll!r}")
        else:
            result.ok("PLAYWRIGHT-4 after poll", f"Chip/chat agreed after poll interval; chip={chip_after_poll!r}")
