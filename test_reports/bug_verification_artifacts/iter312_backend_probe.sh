...
MSG="Complex long planning test for Iter 312: inspect the connected React FastAPI codebase and draft a comprehensive multi-phase implementation plan touching authentication, billing, observability, CI, frontend UX, backend data models, security hardening, rollback strategy, migration plan, E2E tests, and documentation. Do not execute yet; create a careful plan only."
REQ=$(mktemp)
printf '{"project_id":"%s","user_message":"%s"}' "$PROJECT_ID" "$MSG" > "$REQ"
# best-effort cancel existing active for this project
ACTIVE=$(curl -sS "$BASE/api/aurem-dev/loop/active?project_id=$PROJECT_ID" -H "Authorization: Bearer $TOKEN" || true)
...
printf '\n# BACKEND-1 curl: POST /loop/start (async default)\n'
...
printf '\nCaptured loop_id=%s project_id=%s\n' "$LOOP_ID" "$PROJECT_ID"

printf '\n# BACKEND-2 curl: immediate second POST /loop/start same project should 409\n'
RESP2=$(curl -sS -w '\nHTTP_STATUS:%{http_code}\nTIME_TOTAL:%{time_total}\n' -X POST "$BASE/api/aurem-dev/loop/start" \
  -H "Authorization: Bearer $TOKEN" -H 'Content-Type: application/json' --data-binary "@$REQ")
printf '%s\n' "$RESP2"

printf '\n# BACKEND-3 curl: GET /loop/active immediately after async start\n'
RESP3=$(curl -sS -w '\nHTTP_STATUS:%{http_code}\nTIME_TOTAL:%{time_total}\n' "$BASE/api/aurem-dev/loop/active?project_id=$PROJECT_ID" \
  -H "Authorization: Bearer $TOKEN")
printf '%s\n' "$RESP3"

printf '\n# Poll active until plan/terminal (max 180s)\n'
for i in $(seq 1 60); do
  R=$(curl -sS "$BASE/api/aurem-dev/loop/active?project_id=$PROJECT_ID" -H "Authorization: Bearer $TOKEN")
  printf '[poll %02d] %s\n' "$i" "$R"
