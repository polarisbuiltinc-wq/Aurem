...
/* Slice A · BI Cockpit — Live Stripe subscription pull + inference-cost
 * telemetry aggregated from `ora_chat_usage`. Renders "No data yet" for
 * any card with a zero-value LIVE payload (never fake catalog numbers). */
function BiCockpit({ bi, err, reconcile, reconciling, reconcileMsg, onReload }) {
  if (err) {
    return (
  ...
  // 30-day series → recharts-ready
  const series = (i.daily_series_30d || []).map(d => ({
    day: (d.day || "").slice(5), // MM-DD
    cost: Number(d.cost || 0),
    calls: d.calls,
  }));
  // By-model → recharts-ready. Slice long slugs for readability.
...
export default function AdminFinancials() {
  ...
  // Slice A · BI Cockpit — live Stripe + inference-cost payload.
  ...
  const reconcile = useCallback(async () => {
    ...
    try {
      ...
      // Refresh BI + financials so numbers reflect the new state.
      await Promise.all([loadBi(), load()]);
    } catch (e) {
  ...
  useEffect(() => { load(); loadBi(); }, [load, loadBi]);
