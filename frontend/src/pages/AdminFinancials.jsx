...
export default function AdminFinancials() {
  ...
  return (
    <div data-testid="admin-financials-page" style={{
      ...
      {/* 2026-02-18 — Live Business Intelligence panel moved to
          /admin/cockpit as the single source of truth. This page now
          only owns the P&L catalog editor + tier margins + cost-per-task
          tables. See <LiveBusinessIntelligence/> inside AdminCockpit
          for live Stripe MRR, ARR, inference cost, budget mode, and
          the reconcile-orphans button. */}
      <div data-testid="bi-moved-notice" style={{
        fontSize: 12, padding: "10px 14px", marginBottom: 16,
        background: "rgba(94,179,245,0.05)",
        border: "1px solid rgba(94,179,245,0.20)",
        color: "var(--text-dim)", borderRadius: 5,
        display: "flex", justifyContent: "space-between", alignItems: "center",
      }}>
        <span>
          Live BI (MRR, ARR, inference cost, budget mode, reconcile orphans)
          now lives in the <strong>Cockpit</strong> — one dashboard, one source of truth.
        </span>
        <button
          data-testid="bi-open-cockpit-btn"
          onClick={() => nav("/admin/cockpit")}
          style={{
