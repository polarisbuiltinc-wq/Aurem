...
function DeployWidget({ project }) {
  ...
  return (
    <div className="card" data-testid="deploy-widget" style={{ padding: 16 }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 10 }}>
        ...
        <div style={{ display: "flex", gap: 8 }}>
          {connected ? (
            <>
              <button data-testid="deploy-ship" onClick={ship} disabled={busy} className="btn-primary"
                      style={{ padding: "6px 12px", fontSize: 12 }}>
                <Send size={12} /> {busy ? "Shipping…" : "Ship to Live"}
              </button>
              <button data-testid="deploy-disconnect" onClick={disconnect} disabled={busy} className="btn-ghost"
                      style={{ padding: "6px 10px", fontSize: 11 }}>
                Disconnect
              </button>
            </>
          ) : (
            <button data-testid="deploy-connect" onClick={() => setShowCfg(true)} className="btn-primary"
                    style={{ padding: "6px 12px", fontSize: 12 }}>
              Connect deploy
            </button>
          )}
      ...
      {showCfg && (
        <div data-testid="deploy-config" style={{ marginTop: 12, padding: 12, border: "1px solid var(--border)", borderRadius: 8 }}>
          ...
          <div style={{ display: "flex", gap: 8 }}>
            <button data-testid="deploy-connect-save" onClick={connect} disabled={busy || !hookUrl.trim()}
